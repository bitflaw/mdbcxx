...
std::optional<Result> Transaction::exec(std::string sql)
{
  if (!cxn.is_connected())
  {
    throw std::runtime_error("Connection handle is NULL!");
  }
  if (mysql_real_query(cxn.raw(), sql.data(), sql.size()))
  {
    throw std::runtime_error(mysql_error(cxn.raw()));
  }
...
{
  if (pstmt.stmt.get() == NULL)
  {
    throw std::runtime_error("Prepared statement handle is NULL!");
  }
  if (p.size() != pstmt.param_count)
  {
    throw std::length_error(
      std::format("Expected {} param(s), got {}", pstmt.param_count, p.size())
    );
  }
  ...
  {
    throw std::runtime_error(mysql_stmt_error(pstmt.stmt.get()));
  }
  ...
  if(fields == NULL)
  {
    throw std::runtime_error(
      std::format("Expected a field array of len {}, instead got NULL!", nfields)
    );
  }
...
void Transaction::exec0(std::string sql)
{
  ...
  {
    throw std::length_error(
      std::format("Expected 0 rows, got {}", nrows)
    );
  }
}
...
void Transaction::exec0(prepped_stmt& pstmt, params p)
{
  ...
  {
    throw std::length_error(
      std::format("Expected {} params, got {}", pstmt.param_count, p.size())
    );
  }
  std::vector<MYSQL_BIND> pbstructs {};
  pbstructs.reserve(p.size());
...
Row Transaction::exec1(std::string sql)
{
  ...
  {
    throw std::length_error("Expected 1 row, got 0");
  }
  ...
  {
    throw std::length_error(
      std::format("Expected 1 row, got {}", res.size())
    );
  }
  return res[0];
}
...
Row Transaction::exec1(prepped_stmt& pstmt, params p)
{
  ...
  {
    throw std::length_error(
      std::format("Expected {} params, got {}",
  ...
  if(nfields == 0)
  {
    throw std::length_error("Expected a result set, got none!");
  }
...
std::optional<Result> Transaction::execn(std::size_t n, std::string sql)
{
  ...
  {
    throw std::length_error(
      std::format("Expected {} rows, got {}", n, res.size())
    );
  }
  return res;
}

// NOTE: there is a concept of streaming, which i intend to add.
...
std::optional<Result> Transaction::execn(std::size_t n, prepped_stmt& pstmt, params p)
{
  ...
  {
    throw std::runtime_error(
      std::format("Expected a field array of length {}, got NULL!", nfields)
    );
  }
  init_rbstructs(rbstructs, fields);
...
{
  if(mysql_commit(cxn.raw())) throw std::runtime_error(mysql_error(cxn.raw()));
}

void Transaction::rollback()
{
  if(mysql_rollback(cxn.raw())) throw std::runtime_error(mysql_error(cxn.raw()));
}
